    ...
    <div class="col-xs-12 margin-top-20 padding-0">
        ...
        <div class="col-xs-12 padding-0">
            <div class="col-xs-12 col-md-6 thumbnail margin-right-md-20 margin-bottom-20">
                <img src="<?php print file_create_url($field_process_image[0]['uri']); ?>" alt="<?php print $field_process_image[0]['alt'] ?>" title="<?php print $field_process_image[0]['alt'] ?>" />
            </div>
            <div class="text-justify"><?php print $field_process_text[0]['value']  ?></div>
            <div class="col-xs-12 padding-0">
                <div class="hidden-text-area"><?php print $field_process_more_text[0]['value']  ?></div>
        ...
        foreach ($raws as $raw) { ?>
            <div class="col-xs-12 padding-0">
                ...
                <div class="gallery gallery-container"><?php
                    ...
                    $list_captioned_youtube_ids = [];
                    foreach ($raw->field_video_youtube_captioned['und'] as $item_id) {
                        $list_captioned_youtube_ids[] = $item_id['value'];
                    }
                    $captioned_youtube_videos = entity_load('field_collection_item', $list_captioned_youtube_ids); 
                    foreach ($captioned_youtube_videos as $captioned_youtube_video) { ?>
                        <div class="col-sm-3 padding">
                            <a class="thumbnail popup-youtube" title="<?php print $captioned_youtube_video->field_image['und'][0]['title'] ?>" href="<?php print $captioned_youtube_video->field_youtube_url['und'][0]['value']; ?>">
                                <img class="" src="<?php print file_create_url($captioned_youtube_video->field_image['und'][0]['uri']); ?>" alt="<?php print $captioned_youtube_video->field_image['und'][0]['alt'] ?>" title="<?php print $captioned_picture->field_image['und'][0]['title'] ?>" />
                            </a>
                            <p class="subtitle-pic text-center"><?php print $captioned_youtube_video->field_caption['und'][0]['value']?></p>
                        </div><?php
                    } ?>
                </div>
            </div><?php
